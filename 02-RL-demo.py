...
with model:
    ...
    with nengo.Network('fpga pes network') as adapt_ens:
    ...
    #     label="pes ensemble",
    # )
        adapt_ens_pre = nengo.Ensemble(
            n_neurons=100 * radar_dim,
            dimensions=radar_dim,
            #learn_rate=learn_rate,
            #function=lambda x: init_transform,
            seed=1524081122,
            label='fpga pre ensemble',
        )
        adapt_ens_err = nengo.Ensemble(
            n_neurons=100 * radar_dim,
            dimensions=3,
            seed=1524081122,
            label='fpga err ensemble',
        )
        adapt_ens_post = nengo.Ensemble(
            n_neurons=100 * radar_dim,
            dimensions=3,
            seed=1524081122,
            label='fpga post ensemble',
        )
        adapt_ens_conn = nengo.Connection(
            pre=adapt_ens_pre,
            post=adapt_ens_post,
            function=lambda x: init_transform,
            learning_rule_type=nengo.PES(learn_rate)
        )
        adapt_ens_err_conn_pre = nengo.Connection(
            pre=adapt_ens_pre,
            post=adapt_ens_err,
            transform=-1
        )
        adapt_ens_err_conn_post = nengo.Connection(
            pre=adapt_ens_post,
            post=adapt_ens_err,
        )
        adapt_ens_conn_learn = nengo.Connection(
            pre=adapt_ens_err,
            post=adapt_ens_conn.learning_rule
        )
        adapt_ens_err.radius = 4

    nengo.Connection(stim_radar, adapt_ens_pre, synapse=learn_synapse)
    nengo.Connection(errors, adapt_ens_err)
    nengo.Connection(adapt_ens_post, bg.input)
